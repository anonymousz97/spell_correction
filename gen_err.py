...
class SynthesizeData(object):
    ...
    def __init__(self, vocab_path="./all-vietnamese-syllables.txt"):
    ...
    def uppercase_random(self, text, onehot_label):
        """
        Randomly uppercases a word in the given text.
        
        Args:
            text (list): The input text as a list of characters.
            onehot_label (list): The one-hot encoded labels for each character in the text.
            
        Returns:
            tuple: A tuple containing a boolean value indicating whether a character was uppercased,
                   the modified text, and the modified one-hot labels.
        """
        # Find index of a character to uppercase
        idx = np.random.randint(0, len(onehot_label))
        prevent_loop = 0
        
        # Loop until a valid character is found
        while onehot_label[idx] > 0 or text[idx].isnumeric() or text[idx] in string.punctuation or text[idx][0].isupper():
            idx = np.random.randint(0, len(onehot_label))
            prevent_loop += 1
        ...
        text[idx] = text[idx].lower().capitalize()
        onehot_label[idx] = 2
        return True, text, onehot_label


    def replace_char_nearby(self, text, onehot_label):
        """
        Replaces a character in the given text with a neighboring character from the keyboard.
        ...
        Returns:
            tuple: A tuple containing a boolean value indicating whether a character was replaced,
                   the modified text, and the modified one-hot labels.
        """
        # Keyboard layout for finding neighboring characters
        keyboard = "qwertyuiop\[\]';lkjhgfdsazxcvbnm,./QWERTYUIOP\{\}\":LKJHGFDSAZXCVBNM<>?"
        
        # Find index of a character to replace
        idx = np.random.randint(0, len(onehot_label))
        prevent_loop = 0
        
        # Loop until a valid character is found
        while onehot_label[idx] > 0 or text[idx].isnumeric() or text[idx] in string.punctuation:
            idx = np.random.randint(0, len(onehot_label))
            prevent_loop += 1
        ...
        # Choose a random character in the word to replace
        idx_char = np.random.randint(0, len(word_noise))
        
        # Replace the character with a neighboring character from the keyboard
        if word_noise[idx_char] in keyboard:
            idx_neighbor = keyboard.index(word_noise[idx_char]) + 1
            if idx_neighbor >= len(keyboard):
                idx_neighbor = keyboard.index(word_noise[idx_char]) - 1
            word_noise = word_noise[: idx_char] + keyboard[idx_neighbor] + word_noise[idx_char + 1:]
    ...
    def remove_diacritics(self, text, onehot_label):
        ...
        idx = np.random.randint(0, len(onehot_label))
        prevent_loop = 0
        while onehot_label[idx] > 0 or text[idx] == unidecode.unidecode(text[idx]) or text[idx] in string.punctuation:
            idx = np.random.randint(0, len(onehot_label))
            prevent_loop += 1
            if prevent_loop > 10:
                return False, text, onehot_label
    ...
    def replace_with_homophone_word(self, text, onehot_label):
        ...
        prevent_loop = 0
        while onehot_label[candidates[idx][0]] > 0:
    ...
    def add_noise(self, sentence, percent_err=0.15, num_type_err=6):
        raw_tokens = self.tokenizer(sentence)
        ...
        for _ in range(0, num_wrong):
            ...
            if err == 0:
                ...
                _, tokens, onehot_label = self.replace_char_nearby(tokens, onehot_label)
            # elif err == 7:
            #     _, tokens, onehot_label = self.uppercase_random(tokens, onehot_label)
            else:
                continue

        for idx, (i,j) in enumerate(zip(raw_tokens, tokens)):
            if i != j and i.lower() not in self.vocab:
                tokens[idx] = i
                onehot_label[idx] = 0
                # print(f"Convert {j} back to {i}") 
            if i == j:
                onehot_label[idx] = 0
